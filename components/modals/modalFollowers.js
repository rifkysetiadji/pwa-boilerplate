...
class modalFollowers extends Component {
    ...
    renderBody=()=>{
      if(this.props.loading.getFollowersLoading && this.props.search.result==null){
        ...
        console.log('kaga nemu')
        return (<div><p style={{color:'black'}}>Hmm,ga nemu</p></div>)
      }
    }
    render() {
        return (
            ...
            <Modal isOpen={this.props.isOpen} fade={false} toggle={this.props.toggle} size="sm" className={this.props.className}>
              <ModalHeader toggle={this.props.toggle}>
                <input className="search-modal" placeholder="Followers"/>
              </ModalHeader>
              <ModalBody>
                {this.renderBody()}
              </ModalBody>
...
export default connect(mapStateToProps)(modalFollowers)